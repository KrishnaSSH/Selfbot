// groq client is provided by dispatcher; no local require needed
...
module.exports = {
  ...
  async execute(message, args, config, groq) {
    ...
    try {
      ...
      const _responseMessage = await message.channel.send(
        `\`\`\`${response}\`\`\``,
      );
      ...
      awaitingReplies.set(_responseMessage.id, userId);
