...
module.exports = {
  ...
  async execute(message, args, config, _groq, _client) {
    ...
    overview.push('');
    overview.push(`Usage: ${config.prefix}help categories | ${config.prefix}help <category> | ${config.prefix}help <command>`);
    return chunkAndSend(message, '```\n' + overview.join('\n') + '\n```');
  }
};
